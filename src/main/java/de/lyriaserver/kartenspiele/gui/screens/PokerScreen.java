...
import de.lyriaserver.kartenspiele.gui.buttons.poker.BuyInButton;
...
public class PokerScreen extends GameScreen<Poker, PokerPlayer> {
    ...
    private final List<IntView> playerIntViews = new ArrayList<>();
    private final List<IntView> spectatorIntViews = new ArrayList<>();
    private TurnIndicator<Poker> turnIndicator;
    @Nullable
    private AmountButton amountButton;
    @Nullable
    private RaiseCallButton raiseCallButton;

    public PokerScreen(Poker game, @Nullable PokerPlayer player) {
        super(game, 54, player);

        if (player == null) setButton(0, new BuyInButton(game));
        for (int i = 0; i < 5; i++) {
            final int finalI = i;
        ...
        if (player != null) {
            ...
            playerIntViews.add(ownMoneyButton);
            ...
            amountButton.propagateUpdate(raiseCallButton);
        }
        else {
        ...
        if (player != null) {
            ...
            playerIntViews.add(placedBetButton);
            for (int i = 0; i < 2; i++) {
                final int finalI = i;
    ...
    public void update() {
        ...
        for (IntView view : playerIntViews) {
            view.update();
        }
        for (IntView view : spectatorIntViews) {
            view.update();
        }
    ...
    public void setSpectator() {
        ...
        this.amountButton = null;
        this.raiseCallButton = null;
        turnIndicator = new TurnIndicator<>(game, null);
        for (int slot = 36; slot < 45; slot++) {
            setButton(slot, turnIndicator);
        }
        setButton(0, new BuyInButton(game));
        playerIntViews.clear();
        cards[5] = null;
        cards[6] = null;
    }
